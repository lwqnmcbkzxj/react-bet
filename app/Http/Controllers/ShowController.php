    ...
    public function forecasts(Request $request) {
        ...
        if ($request->has('subscriptions') && $request['subscriptions'] !== 'false' ) {
            if (Auth::user()) {
                ...
                return $this->sendError('Authorization error', 401);
            }
        }
        ...
        return $response['forecasts']->json();
    }
    ...
    public function rating(Request $request) {
        ...
        if ($request['time'] === 'month') {
            ...
            $query = "SELECT DISTINCT users.*,
                ...
                GROUP BY users.id
                ORDER BY roi DESC";
            $response['rating'] = DB::select($query);
            ...
            $query = "SELECT DISTINCT users.*,
                ...
                GROUP BY users.id
                ORDER BY roi DESC";
            $response['users'] = DB::select($query);
            return $response->json();
        }
    }

    public function posts() {
        $response['posts']= Post::where('is_published', true)->orderBy('id', 'desc')->paginate(10);
        return $response->json();
    }

    public function news() {
        $response['news'] = News::orderBy('id', 'desc')->paginate(10);
        return $response->json();
    }

    public function about() {

        $response['about'] =  Page::where('name', 'about')->first();
        return $response->json();
    }

    public function privacy(){

        $response['page'] =  Page::where('name', 'privacy')->first();
        return $response->json();
    }
...
//    }
    ...
    public function bookmakers() {
        $response['bookmakers'] =  Bookmaker::all();
        return $response->json();
    }
}
